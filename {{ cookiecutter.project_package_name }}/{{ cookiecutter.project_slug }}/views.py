...
class LanguageAnd{{ cookiecutter.project_config_class }}Mixin():
    language_field = 'language'
    namespace_field = '{{ cookiecutter.project_class.lower() }}__{{ cookiecutter.project_config_class.lower() }}__namespace'
    config_model = models.{{ cookiecutter.project_config_class }}
        ...
        if app and app.app_config:
            try:
                with override(self.get_language()):
                    self.namespace = request.{{ cookiecutter.project_class.lower() }}_namespace = resolve(request.path_info).namespace
                    self.config = request.{{ cookiecutter.project_class.lower() }}_config = app.get_config(self.namespace)
            except Resolver404:
                self.namespace = ''
                self.config = None
...
class TranslatableSlugMixin(LanguageAnd{{ cookiecutter.project_config_class }}Mixin):
...
class {{ cookiecutter.project_class }}DetailView(TranslatableSlugMixin, generic.DetailView):
...
class {{ cookiecutter.project_class }}ListView(LanguageAnd{{ cookiecutter.project_config_class }}Mixin, generic.ListView):
    template_name = '{{ cookiecutter.project_slug }}/{{ cookiecutter.project_class.lower() }}_list.html'
